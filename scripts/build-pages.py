...
index='<p class="eyebrow">LE CARNET PRATIQUE</p><h1>Préparer la route.</h1><p class="lead">Dix guides pour organiser le départ, comparer les équipements et garder de la place pour le voyage.</p><div class="guide-grid">'+''.join(cards)+'</div>'
...
(dist/'guides.html').write_text(page('Tous les guides road trip','Dix guides pratiques pour organiser un road trip : bagages, coffre, cuisine compacte, énergie, budget et équipements à comparer avant de partir.',index,path='guides.html',jsonld=gl,back=('','← Accueil')))
info={
